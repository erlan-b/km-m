...
class _OwnerProfileScreenState extends ConsumerState<OwnerProfileScreen> {
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final listingsData = await userRepo.getUserListings(widget.userId, page: 1);
      setState(() {
        _user = user;
  ...
  void _loadMore() {
    ...
    _page++;
    ref.read(publicUsersRepositoryProvider).getUserListings(widget.userId, page: _page).then((data) {
      setState(() {
          ...
          slivers: [
                          ...
                          if (user['city'] != null)
                            Padding(
                              ...
                              child: Row(children: [
                                ...
                                Text(user['city'] as String, style: const TextStyle(color: AppTheme.textSubtle, fontSize: 14)),
                              ]),
                            ),
                          if (createdAt != null)
                            Padding(
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: Container(
                  ...
                  child: Row(
                    children: [
                      _StatItem(label: l.activeListings, value: '${user['listing_count'] ?? 0}'),
                      if (user['seller_type'] != null && user['seller_type'] != 'individual') ...[
                        const SizedBox(width: 24),
                        _StatItem(label: 'Type', value: (user['seller_type'] as String).toUpperCase()),
                      ],
                    ],
                  ),
                ),
            ...
            SliverToBoxAdapter(
              child: Padding(
                ...
                child: Text(l.listings, style: const TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
              ),
            ),
            ...
            if (_listings.isEmpty)
              SliverFillRemaining(
                child: Center(child: Text(l.emptyList, style: const TextStyle(color: AppTheme.textSubtle))),
              )
            else
              SliverPadding(
